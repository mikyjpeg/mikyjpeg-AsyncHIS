...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('add_squadrons_to_seazone')
        .setDescription('Add naval squadron(s) to a sea zone')
        ...
        .addStringOption(option =>
            option.setName('seazone')
                .setDescription('Sea zone to add the squadron to')
                .setRequired(true)
                .addChoices(
    ...
    async execute(interaction) {
        try {
            ...
            const historyEntry = await commandHistory(channelName).recordSlashCommand(
                interaction,
                COMMAND_TYPES.ADD_SQUADRONS_TO_SEAZONE,
                {
                    seaZoneName,
                    squadron
                }
            ...
            let message = `Added ${number} squadron(s) for ${power} to ${seaZoneName}`;
            if (loaned) {
