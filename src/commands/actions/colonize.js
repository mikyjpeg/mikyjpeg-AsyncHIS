...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            await am.validateAction('colonize', power);

            // Get the action cost
            const cost = await am.getActionCost('colonize', power);

            // Record in command history
            const historyEntry = await commandHistory(channelName).recordSlashCommand(
                interaction,
                COMMAND_TYPES.ACTION_COLONIZE,
                {
                    actionId: 'colonize',
                    power,
            ...
            await interaction.reply({
                content: `Action: colonize\nPower: ${power}\n(Cost: ${cost})\n(Command ID: ${historyEntry.commandId})`,
                ephemeral: true
            });

        } catch (error) {
            console.error('Error in colonize command:', error);
            await interaction.reply({
