...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('call_theological_debate')
        .setDescription('Initiate theological debate in one language zone.')
    ...
    async execute(interaction) {
        try {
            ...
            await am.validateAction('call_theological_debate', power);

            // Get the action cost
            const cost = await am.getActionCost('call_theological_debate', power);

            // Record in command history
            const historyEntry = await commandHistory(channelName).recordSlashCommand(
                interaction,
                COMMAND_TYPES.ACTION_CALL_THEOLOGICAL_DEBATE,
                {
                    actionId: 'call_theological_debate',
                    power,
            ...
            await interaction.reply({
                content: `Action: call_theological_debate\nPower: ${power}\n(Cost: ${cost})\n(Command ID: ${historyEntry.commandId})`,
                ephemeral: true
            });

        } catch (error) {
            console.error('Error in call_theological_debate command:', error);
            await interaction.reply({
